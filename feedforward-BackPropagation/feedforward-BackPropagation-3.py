...
class FullyConnect:
    ...
    def backward(self, d):
        sample_num = self.x.shape[1]
        self.dw = (d * self.x).mean(axis=1)  # 根据链式法则，将反向传递回来的导数值乘以x，针对样本求平均后，得到对参数的梯度
        self.db = d.mean(axis=1)
...
def main():
        ...
        # 执行反向传播
        d1 = sigmoid.backward([1,2,3]) #先假定3次损失对y'的导数为1,2,3
        dx = fc.backward(d1)
        dx11 = sigmoid11.backward(dx[0])
        dx12 = sigmoid12.backward(dx[1])
